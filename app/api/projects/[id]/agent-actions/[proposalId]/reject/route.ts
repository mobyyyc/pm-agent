...
import { rejectAgentActionProposal } from "@/lib/agent-actions/action-registry";
...
import { rejectProjectAgentActionProposalRequestSchema } from "@/lib/validators";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    if (normalizeUserId(project.userId) !== sessionUserId) {
      return NextResponse.json({ error: "Only the project owner can reject agent actions." }, { status: 403 });
    }
    ...
    const parsed = rejectProjectAgentActionProposalRequestSchema.parse(body);
    ...
    return NextResponse.json(
      {
        error: "Failed to reject agent action proposal.",
        detail: message,
      },
      { status },
    );
