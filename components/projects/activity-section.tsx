...
export function ActivitySection({ events, projectMembers, isGuest }: ActivitySectionProps) {
  ...
  return (
    <section className="app-frame rounded-2xl bg-white/5 p-4 sm:p-5 md:p-6">
      <div className="mb-4 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold tracking-tight text-white">Activity</h2>
          {isGuest ? (
      ...
      {events.length === 0 ? (
        ...
        <ul className="space-y-3">
          {events.map((event) => (
            <li key={event.id} className="app-frame-item app-frame-hover rounded-xl bg-white/5 p-4 transition-colors">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  <p className="wrap-break-word text-sm font-semibold text-white">{event.summary}</p>
                  <div className="mt-2 flex min-w-0 flex-wrap gap-2 text-xs text-neutral-500">
